...
class LoginActivity : AppCompatActivity(), View.OnClickListener {
        ...
        mAuth!!.createUserWithEmailAndPassword(email, password)
                .addOnCompleteListener(this) { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(applicationContext, R.string.authFail, Toast.LENGTH_SHORT).show()
                        updateUI(null)
                    }
                }
    }
    ...
    private fun sendEmailVerification() {
        ...
        user!!.sendEmailVerification()
                .addOnCompleteListener(this) { task ->
                    ...
                    if (task.isSuccessful) {
                        Toast.makeText(applicationContext, R.string.verifyEmail, Toast.LENGTH_SHORT).show()
                    } else {
                        Toast.makeText(applicationContext, R.string.sendEmailFail.toString(), Toast.LENGTH_SHORT).show()
                    }
                }
    }
    ...
    private fun validateForm(email: String, password: String): Boolean {
        ...
        if (TextUtils.isEmpty(password)) {
            Toast.makeText(applicationContext, R.string.enterPassword, Toast.LENGTH_SHORT).show()
            return false
        }

        if (password.length < 6) {
            Toast.makeText(applicationContext, R.string.minPassword, Toast.LENGTH_SHORT).show()
            return false
        }
